		...
		<div class="main_body">
            ...
            <div style="text-align: center; font-size: 18px; color: red; min-height: 10px;">
				...
				try {
					...
					$result6 = $conn->query("SELECT sum(costAmount) as salary FROM `costtable` WHERE costType='salary' AND entryDate BETWEEN  '$fromDate' AND '$toDate'");
					while ($row = $result6->fetch())
						{ 
							$salary=$row['salary']; 
						}	
					...
					$result6 = $conn->query("SELECT sum(costAmount) as teacherPayment FROM `costtable` WHERE costType='Teacher Payment' AND entryDate BETWEEN  '$fromDate' AND '$toDate'");
					while ($row = $result6->fetch())
						{   
					...
					$result6 = $conn->query("SELECT sum(costAmount) as staffPayment FROM `costtable` WHERE costType='Staff Payment' AND entryDate BETWEEN '$fromDate' AND '$toDate'");
					while ($row = $result6->fetch())
						{   
					...
					$result6 = $conn->query("SELECT sum(costAmount) as officeRent FROM `costtable` WHERE costType='Office Rent' AND entryDate BETWEEN  '$fromDate' AND '$toDate'");
					while ($row = $result6->fetch())
						{   
					...
					$result6 = $conn->query("SELECT sum(costAmount) as officeEquipment FROM `costtable` WHERE costType='Office Equipment' AND entryDate BETWEEN  '$fromDate' AND '$toDate'");
					while ($row = $result6->fetch())
						{   
					...
					$result6 = $conn->query("SELECT sum(costAmount) as others FROM `costtable` WHERE costType NOT IN ('Teacher Payment','Staff Payment','Office Rent','Office Equipment','salary') AND entryDate BETWEEN  '$fromDate' AND '$toDate'");
					while ($row = $result6->fetch())
						{   
							$others=$row['others']; 
						}	
            ...
            <div class="adminform" style="width:80%;">
                ...
                <div style="height: 40px;">
                    <form action="revenuePrintByDate.php">
