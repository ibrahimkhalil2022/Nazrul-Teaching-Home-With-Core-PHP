...
<html>
    ...
    <body style="background-color: darkorchid;">
				...
				try {
					...
					$result1 = $conn->query("SELECT sum(payment) as admission FROM `incometable` WHERE incomeType='Admission'"); 
					...
					$result2 = $conn->query("SELECT sum(payment) as bookSell FROM `incometable` WHERE incomeType='Book Sell'"); 
					...
					$result3 = $conn->query("SELECT sum(payment) as mockTest FROM `incometable` WHERE incomeType='MOCK Test'"); 
					...
					$result4 = $conn->query("SELECT sum(payment) as transferFee FROM `incometable` WHERE incomeType='Transfer Fee'"); 
					...
					$result5 = $conn->query("SELECT sum(payment) as reAdmission FROM `incometable` WHERE incomeType='reAdmission'"); 
					...
					$result6 = $conn->query("SELECT sum(payment) as certificateFee FROM `incometable` WHERE incomeType='Certificate Fee'");
					...
					$result7 = $conn->query("SELECT sum(costAmount) as salary FROM `costtable` WHERE costType='salary'");
					...
					$result8 = $conn->query("SELECT sum(costAmount) as teacherPayment FROM `costtable` WHERE costType='Teacher Payment'");
					...
					$result9 = $conn->query("SELECT sum(costAmount) as staffPayment FROM `costtable` WHERE costType='Staff Payment'");
					...
					$result10 = $conn->query("SELECT sum(costAmount) as officeRent FROM `costtable` WHERE costType='Office Rent'");
					...
					$result11 = $conn->query("SELECT sum(costAmount) as officeEquipment FROM `costtable` WHERE costType='Office Equipment'");
					...
					$result12 = $conn->query("SELECT sum(costAmount) as others FROM `costtable` WHERE costType NOT IN ('Teacher Payment','Staff Payment','Office Rent','Office Equipment','salary') ");
					...
					$result13 = $conn->query("SELECT MIN(id) as id FROM `transaction`"); 
					 
					 while ($row = $result13->fetch()) 
						{  
							$id=$row['id']; 
						}	
						
					$result14 = $conn->query("SELECT date FROM `transaction` where id = '$id'"); 
					 
					while ($row = $result14->fetch()) 
						{  
							$fromDate=$row['date']; 
						}
...
<!--                    <div class="dateSerial" style="text-align:left;">MR No.&nbsp;<b><?php echo $mrpNo?></b></div>-->
                    <div class="dateSerial" style="margin-left:280px; border-bottom: 1px solid black;">
                        <font style="font-weight: bolder; font-size: 18px;">Company's Revenue</font><br>
                    </div>
                    <div class="dateSerial" style="float:right;">Date:&nbsp;&nbsp;<b><?php echo $d = date("d, M Y");?></b></div>
                </div>
                <div style="text-align: center;">
                    <b>From:&nbsp;&nbsp;</b><?php echo $fromDate;?>&nbsp;&nbsp;<b>to&nbsp;&nbsp;</b> <?php echo $date;?>.
                </div>
                ...
                <div style="width:80%; margin:auto">
                    ...
                    <div style="margin-top: 15px; font-weight: bold;">
                        This report is generated from date <b><?php echo $fromDate;?></b> to<b> <?php echo $date;?>.</b>
                    </div>
